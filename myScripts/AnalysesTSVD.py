...
class Analyses:
    def __init__(self, df):
        ...
        self.dimen_reduc_plot(train_data_cv, target_vect, 1)

    def tfidf(self, data):
        tfidfv = TfidfVectorizer(dtype=np.float32, ngram_range=(2, 3), max_df=0.75, stop_words="english",
                                 max_features=200000)
        fit_data_tfidf = tfidfv.fit_transform(data)
        print(fit_data_tfidf.shape)
        return fit_data_tfidf, tfidfv

    def dimen_reduc_plot(self, test_data, test_label, option):
        tsvd = TruncatedSVD(n_components=100, algorithm="randomized", n_iter=15, n_oversamples=10, random_state=24)
